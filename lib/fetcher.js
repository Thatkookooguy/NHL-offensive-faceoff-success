...
const limiter = new Bottleneck({
  // allow a total of 20 requests in parallel to not hit the rate limit
  maxConcurrent: 20,
  // assume each request takes a minimum of 333 milliseconds
  minTime: 333
});
...
function getGamesData(gameUrls) {
    ...
    return Promise.all(allPromises)
      ...
      .then((results) => results.filter((result) => result && result.error));
}
